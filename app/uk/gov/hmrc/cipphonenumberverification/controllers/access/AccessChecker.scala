...
trait AccessChecker {
  ...
  private val accessRequestFormUrl: String = appConfig.mustGetConfig[String](accessRequestFormUrlAbsoluteKey)

  private val checkAllowList: Boolean = appConfig.mustGetConfig[String](accessControlEnabledAbsoluteKey).toBoolean

  private val allowedClients: Set[String] = appConfig
    .getConfig[Seq[String]](accessControlAllowListAbsoluteKey)
    .getOrElse(if (checkAllowList) throw new RuntimeException(s"Could not find config $accessControlAllowListAbsoluteKey") else Seq())
    .toSet
  ...
  private def forbiddenResponse(clients: Seq[String]): String =
    s"""{
       |"code": 403,
       |"description": "One or more user agents in '${clients
      .mkString(",")}' are not authorized to use this service. Please complete '$accessRequestFormUrl' to request access."
       |}""".stripMargin
