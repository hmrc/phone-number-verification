...
class VerifyServiceSpec extends AnyWordSpec
  ...
  "verify" should {
    "return success if telephone number is valid" in new SetUp {
      ...
      passcodeGeneratorMock.generatePasscode() was called
  ...
  trait SetUp {
    ...
    val passcodeServiceMock: VerifyPasscodeService = mock[VerifyPasscodeService]
    ...
    val passcode = "ABCDEF"
    passcodeGeneratorMock.generatePasscode().returns(passcode)
